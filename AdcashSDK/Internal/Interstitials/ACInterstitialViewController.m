//
//  ACInterstitialViewController.m
//  Adcash
//
...
static NSString * const kCloseButtonXImageName = @"ACCloseButtonX.png";

@interface ACInterstitialViewController ()
...
- (void)setCloseButtonStyle:(ACInterstitialCloseButtonStyle)style;
...
@implementation ACInterstitialViewController
...
- (void)presentInterstitialFromViewController:(UIViewController *)controller
{
    if (_isOnViewControllerStack) {
        ACLogWarn(@"Cannot present an interstitial that is already on-screen.");
        return;
    }
    ...
    [controller mp_presentModalViewController:self animated:AC_ANIMATED];
}
...
- (void)setCloseButtonStyle:(ACInterstitialCloseButtonStyle)style
{
    ...
    switch (style) {
        case ACInterstitialCloseButtonStyleAlwaysVisible:
            self.closeButton.hidden = NO;
            break;
        case ACInterstitialCloseButtonStyleAlwaysHidden:
            self.closeButton.hidden = YES;
            break;
        case ACInterstitialCloseButtonStyleAdControlled:
...
- (void)dismissInterstitialAnimated:(BOOL)animated
{
    ...
    if ([presentingViewController mp_presentedViewController] == self) {
        [presentingViewController mp_dismissModalViewControllerAnimated:AC_ANIMATED];
    }
...
- (void)setApplicationStatusBarHidden:(BOOL)hidden
{
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= AC_IOS_3_2
...
- (NSUInteger)supportedInterfaceOrientations
{
    ...
    [[UIApplication sharedApplication] supportedInterfaceOrientationsForWindow:ACKeyWindow()];
    ...
    if (_orientationType == ACInterstitialOrientationTypePortrait) {
        interstitialSupportedOrientations &=
        (UIInterfaceOrientationMaskPortrait | UIInterfaceOrientationMaskPortraitUpsideDown);
        orientationDescription = @"portrait";
    }
    else if (_orientationType == ACInterstitialOrientationTypeLandscape) {
    ...
    if (!interstitialSupportedOrientations) {
        ACLogError(@"Your application does not support this interstitial's desired orientation "
...
- (UIInterfaceOrientation)preferredInterfaceOrientationForPresentation
{
    ...
    UIInterfaceOrientation currentInterfaceOrientation = ACInterfaceOrientation();
