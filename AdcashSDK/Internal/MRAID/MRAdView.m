...
static NSString *const kExpandableCloseButtonImageName = @"ACCloseButtonX.png";
static NSString *const kMraidURLScheme = @"mraid";
static NSString *const kAdcashURLScheme = @"adcash";
static NSString *const kAdcashPrecacheCompleteHost = @"precacheComplete";
static NSString *const kAdcashClose = @"close";
   ...
   closeButtonStyle:(MRAdViewCloseButtonStyle)style placementType:(MRAdViewPlacementType)type
{
    ...
    if (self) {
        ...
        _webView = [[[ACInstanceProvider sharedProvider] buildUIWebViewWithFrame:frame] retain];
        ...
        _displayController = [[MRAdViewDisplayController alloc] initWithAdView:self
                                                              ...
                                                              closeButtonStyle:style
                                                               jsEventEmitter:[[ACInstanceProvider sharedProvider] buildMRJavaScriptEventEmitterWithWebView:_webView]];
        ...
        _destinationDisplayAgent = [[[ACCoreInstanceProvider sharedProvider]
                                    buildACAdDestinationDisplayAgentWithDelegate:self] retain];
        _calendarManager = [[[ACInstanceProvider sharedProvider]
                             buildMRCalendarManagerWithDelegate:self] retain];
        _pictureManager = [[[ACInstanceProvider sharedProvider]
                             buildMRPictureManagerWithDelegate:self] retain];
        _videoPlayerManager = [[[ACInstanceProvider sharedProvider]
                                buildMRVideoPlayerManagerWithDelegate:self] retain];
        _jsEventEmitter = [[[ACInstanceProvider sharedProvider]
...
{
     ACLogDebug(@"MRAdView (%p) uses custom close button", self);
...
- (BOOL)isViewable
{
    return ACViewIsVisible(self);
}

- (void)loadCreativeFromURL:(NSURL *)url
{
     ACLogDebug(@"MRAdView (%p) starting to load creative from URL", self);
...
{
     ACLogDebug(@"MRAdView (%p) starting to load creative with HTML string", self);
...
- (void)loadHTMLString:(NSString *)string baseURL:(NSURL *)baseURL
{
    ...
    if (![self MRAIDScriptPath]) {
         ACLogDebug(@"Failed to load MRAIDScriptPath.");
   ...
   //     [_webView disableJavaScriptDialogs];
         ACLogDebug(@"Loading HTML string with baseURL.");
...
- (NSMutableString *)HTMLWithJavaScriptBridge:(NSString *)HTML
{
    ...
    if ([self HTMLStringIsMRAIDFragment:HTML]) {
        ACLogDebug(@"Fragment detected: converting to full payload.");
...
- (NSString *)MRAIDScriptPath
{
    MRBundleManager *bundleManager = [[ACInstanceProvider sharedProvider] buildMRBundleManager];
...
- (void)initializeJavascriptState
{
    ACLogDebug(@"Injecting initial JavaScript state.");
...
- (void)handleCommandWithURL:(NSURL *)URL
{
    ...
    NSDictionary *parameters = ACDictionaryFromQueryString(URL.query);
    BOOL success = YES;
     
     ACLogDebug(@"handleCommandWithURL (%p) is called with command %@", self, command);
    ...
    if (!success) {
        ACLogDebug(@"Unknown command: %@", command);
...
{
    ACLogDebug(@"MRAdView - loading Adcash URL: %@", url);
    NSString *host = [url host];
    if ([host isEqualToString:kAdcashPrecacheCompleteHost] && self.adType == MRAdViewAdTypePreCached) {
        [self adDidLoad];
    }
    else if ([host isEqualToString:kAdcashClose]) {
         ACLogDebug(@"MRAdView - closing ad using Adcash URL: %@", url);

              [self adDidClose];
    } else {
        ACLogWarn(@"MRAdView - unsupported Adcash URL: %@", [url absoluteString]);
    }
}
 ...
 navigationType:(UIWebViewNavigationType)navigationType
{
    ...
    ACLogDebug(@"Trying to load URL from MRAdView: %@", urlString);

    if ([scheme isEqualToString:kMraidURLScheme]) {
        ACLogDebug(@"Trying to process Mraid command: %@", urlString);
        [self handleCommandWithURL:url];
        return NO;
    } else if ([scheme isEqualToString:kAdcashURLScheme]) {
         ACLogDebug(@"Trying to process Adcash command: %@", urlString);
        ...
        ACLogDebug(@"Web console: %@", urlString);
        return NO;
    }
