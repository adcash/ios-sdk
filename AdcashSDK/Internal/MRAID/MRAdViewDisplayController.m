     ...
     jsEventEmitter:(MRJavaScriptEventEmitter *)jsEventEmitter
{
    ...
    if (self) {
        ...
        _viewabilityTimer = [[[ACCoreInstanceProvider sharedProvider] buildACTimerWithTimeInterval:kViewabilityTimerInterval
                                                                                        target:self
        ...
        _dimmingView = [[MRDimmingView alloc] initWithFrame:ACKeyWindow().frame];
...
- (void)initializeJavascriptStateWithViewProperties:(NSArray *)viewProperties
{
    ...
    properties = [properties arrayByAddingObjectsFromArray:[NSArray arrayWithObjects:
            [MRScreenSizeProperty propertyWithSize:ACApplicationFrame().size],
...
- (void)expandToFrame:(CGRect)frame withURL:(NSURL *)url blockingColor:(UIColor *)blockingColor
    ...
    [ACKeyWindow() addSubview:_dimmingView];
    
    if (url) {
        self.twoPartExpansionView = [[ACInstanceProvider sharedProvider] buildMRAdViewWithFrame:self.view.frame
                                                                                allowsExpansion:NO
...
- (void)assignRandomTagToDefaultSuperview {
    ...
    } while ([ACKeyWindow() viewWithTag:_parentTag]);
    
    self.view.superview.tag = _parentTag;
}

- (void)restoreDefaultSuperviewTag {
    [[ACKeyWindow() viewWithTag:_parentTag] setTag:_originalTag];
}
...
- (void)moveViewFromWindowToDefaultSuperview {
    UIView *defaultSuperview = [ACKeyWindow() viewWithTag:_parentTag];
...
- (void)constrainViewBoundsToApplicationFrame {
    ...
    CGRect applicationFrame = ACApplicationFrame();
...
- (void)updateViewabilityWithBool:(BOOL)currentViewability {
    if (_isViewable != currentViewability) {
        ACLogDebug(@"Viewable changed to: %@", currentViewability ? @"YES" : @"NO");
