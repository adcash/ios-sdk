...
- (void)dealloc
{
    ...
    // XXX:
    [_eventEditViewController setEditViewDelegate:[ACLastResortDelegate sharedDelegate]];
...
- (void)createCalendarEventWithParameters:(NSDictionary *)parameters
{
    if (!self.eventEditViewController) {
        self.eventEditViewController = [[ACInstanceProvider sharedProvider] buildEKEventEditViewControllerWithEditViewDelegate:self];
    }
