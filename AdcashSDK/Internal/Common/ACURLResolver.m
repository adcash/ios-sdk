...
static NSString * const kAdcashSafariScheme = @"adcashnativebrowser";
static NSString * const kAdcashSafariNavigateHost = @"navigate";
static NSString * const kAdcashHTTPHeaderContentType = @"Content-Type";

@interface ACURLResolver ()
...
{
    return [[[ACURLResolver alloc] init] autorelease];
}
...
- (void)startResolvingWithURL:(NSURL *)URL delegate:(id<ACURLResolverDelegate>)delegate
{
    ...
    if (![self handleURL:self.URL]) {
        NSURLRequest *request = [[ACCoreInstanceProvider sharedProvider] buildConfiguredURLRequestWithURL:self.URL];
...
- (BOOL)handleURL:(NSURL *)URL
{
    if ([self storeItemIdentifierForURL:URL]) {
        ...
        if ([[UIApplication sharedApplication] canOpenURL:URL]) {
            ...
            [self.delegate failedToResolveURLWithError:[NSError errorWithDomain:@"com.adcash" code:-1 userInfo:nil]];
        }
    } else {
        return NO;
    }
...
- (NSString *)safariURLForURL:(NSURL *)URL
{
    ...
    if ([[URL scheme] isEqualToString:kAdcashSafariScheme] &&
        [[URL host] isEqualToString:kAdcashSafariNavigateHost]) {
...
- (NSStringEncoding)stringEncodingFromContentType:(NSString *)contentType
{
    ...
    if (![contentType length]) {
        ACLogWarn(@"Attempting to set string encoding from nil %@", kAdcashHTTPHeaderContentType);
        return encoding;
    }
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response
{
    ...
    NSString *contentType = [headers objectForKey:kAdcashHTTPHeaderContentType];
