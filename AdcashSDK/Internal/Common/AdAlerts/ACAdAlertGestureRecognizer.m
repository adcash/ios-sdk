//
//  ACAdAlertGestureRecognizer.m
//  Adcash
//
//  Copyright (c) 2013 Adcash. All rights reserved.
//

#import "ACAdAlertGestureRecognizer.h"
...
NSInteger const kACAdAlertGestureMaxAllowedYAxisMovement = 50;

@interface ACAdAlertGestureRecognizer ()

@property (nonatomic, assign) ACAdAlertGestureRecognizerState currentAlertGestureState;
...
@implementation ACAdAlertGestureRecognizer
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if((self.state == UIGestureRecognizerStatePossible) && self.currentAlertGestureState == ACAdAlertGestureRecognizerState_Recognized)
    {
...
{
    self.currentAlertGestureState = ACAdAlertGestureRecognizerState_ZigRight1;
...
- (void)handleZigRightGestureStateWithTouches:(NSSet *)touches
{
    ...
    else if(nowPoint.x < prevPoint.x) // user has changed touch direction
    {
        if(self.thresholdReached)
        {
            self.inflectionPoint = nowPoint;
            self.currentAlertGestureState = ACAdAlertGestureRecognizerState_ZagLeft2;
            self.thresholdReached = NO;
        }
        else // the user changed directions before covering the required distance, fail
        {
            self.state = UIGestureRecognizerStateFailed;
        }
    }
...
- (void)handleZagLeftGestureStateWithTouches:(NSSet *)touches
{
    ...
    if(nowPoint.x < prevPoint.x && self.inflectionPoint.x - nowPoint.x >= self.minTrackedDistanceForZigZag)
    {
        ...
        {
            self.currentAlertGestureState = ACAdAlertGestureRecognizerState_Recognized;
        }
    }
    else if(nowPoint.x > prevPoint.x) // user has changed touch direction
    {
...
- (void)updateAlertGestureStateWithTouches:(NSSet *)touches
{
    ...
    if(![self touchIsWithinBoundsForTouches:touches] && self.currentAlertGestureState != ACAdAlertGestureRecognizerState_Recognized)
    {
        self.state = UIGestureRecognizerStateFailed;
        return;
    }
    ...
    {
        case ACAdAlertGestureRecognizerState_ZigRight1:
        {
            [self handleZigRightGestureStateWithTouches:touches];
            
            break;
        }
        case ACAdAlertGestureRecognizerState_ZagLeft2:
        {
...
- (BOOL)validYAxisMovementForTouches:(NSSet *)touches
{
    ...
    return abs(nowPoint.y - self.startingPoint.y) <= kACAdAlertGestureMaxAllowedYAxisMovement;
}
