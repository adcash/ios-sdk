...
@interface ACAdAlertManager () <UIGestureRecognizerDelegate, MFMailComposeViewControllerDelegate>

@property (nonatomic, assign) BOOL processedAlert;
@property (nonatomic, retain) ACAdAlertGestureRecognizer *adAlertGestureRecognizer;
@property (nonatomic, retain) MFMailComposeViewController *currentOpenMailVC;

@end

@implementation ACAdAlertManager
...
- (id)init
{
    ...
    if(self != nil)
    {
        self.adAlertGestureRecognizer = [[ACCoreInstanceProvider sharedProvider] buildACAdAlertGestureRecognizerWithTarget:self
...
- (void)dealloc
{
    ...
    self.adUnitId = nil;
    self.location = nil;
    self.currentOpenMailVC.mailComposeDelegate = [ACLastResortDelegate sharedDelegate];
...
- (void)processAdAlert
{
    ...
    ACLogInfo(@"ACAdAlertManager processing ad alert");
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^
    {
        ...
        dispatch_async(dispatch_get_main_queue(), ^
        {
            ...
            [params setValue:[ACIdentityProvider identifier] forKey:@"device_id"];
            ...
            [params setValue:AC_SDK_VERSION forKey:@"sdk_version"];
            ...
            ACLogInfo(@"ACAdAlertManager finished processing ad alert");
        });
    });
}
...
{
    ACLogInfo(@"ACAdAlertManager alert gesture recognized");
...
- (void)processAdParams:(NSDictionary *)params andScreenshot:(UIImage *)screenshot
{
    ...
    [self.currentOpenMailVC setToRecipients:[NSArray arrayWithObject:@"creative-review@adcash.com"]];
    ...
    [[self.delegate viewControllerForPresentingMailVC] mp_presentModalViewController:self.currentOpenMailVC animated:AC_ANIMATED];
...
- (void)mailComposeController:(MFMailComposeViewController*)controller didFinishWithResult:(MFMailComposeResult)result error:(NSError*)error
{
    ...
    [[self.delegate viewControllerForPresentingMailVC] mp_dismissModalViewControllerAnimated:AC_ANIMATED];
}
