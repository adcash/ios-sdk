...
/**
 * The `ACNativeAdAdapter` protocol allows the Adcash SDK to interact with native ad objects obtained
 * from third-party ad networks. An object that adopts this protocol acts as a wrapper for a native
 * ad object, translating its proprietary interface into a common one that the Adcash SDK can
 * understand.
 *
 ...
 * the pre-defined keys in `ACNativeAdConstants`.
 *
 ...
 */
@protocol ACNativeAdAdapter <NSObject>
 ...
 *
 * When a view containing a native ad is rendered and presented, the Adcash SDK begins tracking the
