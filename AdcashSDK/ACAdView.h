...
typedef enum
{
    ACNativeAdOrientationAny,
    ACNativeAdOrientationPortrait,
    ACNativeAdOrientationLandscape
} ACNativeAdOrientation;

@protocol ACAdViewDelegate;

/**
 * The ACAdView class provides a view that can display banner advertisements.
 */
...
/**
 * Initializes an ACAdView with the given ad unit ID and banner size.
 *
 * @param adUnitId A string representing a Adcash ad unit ID.
 * @param size The desired ad size. A list of standard ad sizes is available in ACConstants.h.
...
/**
 * The delegate (`ACAdViewDelegate`) of the ad view.
 *
 * @warning **Important**: Before releasing an instance of `ACAdView`, you must set its delegate
 * property to `nil`.
 */
@property (nonatomic, assign) id<ACAdViewDelegate> delegate;

/** @name Setting Request Parameters */

/**
 * The Adcash ad unit ID for this ad view.
 *
...
/**
 * Requests a new ad from the Adcash ad server.
 *
...
/**
 * Cancels any existing ad requests and requests a new ad from the Adcash ad server.
 */
- (void)forceRefreshAd;
 ...
 * if you call this with ACInterfaceOrientationPortrait, native networks (e.g. iAd) will never
 * return ads sized for the landscape orientation.
 *
 * @param orientation An ACNativeAdOrientation enum value.
 *
...
- (void)lockNativeAdsToOrientation:(ACNativeAdOrientation)orientation;
 ...
 * has been configured on the Adcash website. This method prevents new ads from automatically loading,
 ...
 * refresh settings on the Adcash website.
 *
 ...
 * automatically load new advertisements if a refresh interval has been configured on the Adcash
 ...
 * events using a subclass of `ACBannerCustomEvent`.
 */
...
/**
 * Replaces the content of the ACAdView with the specified view.
 *
 * @bug **Warning**: This method has been deprecated. You should instead implement banner custom
 * events using a subclass of `ACBannerCustomEvent`.
 *
...
/**
 * A Boolean value that determines whether the ad view should ignore directions from the Adcash
...
/**
 * The delegate of an `ACAdView` object must adopt the `ACAdViewDelegate` protocol. It must
...
@protocol ACAdViewDelegate <NSObject>
 ...
 */
- (void)adViewDidLoadAd:(ACAdView *)view;
 ...
 * @param view The ad view sending the message.
 */
- (void)adViewDidFailToLoadAd:(ACAdView *)view;
 ...
 */
- (void)willPresentModalViewForAd:(ACAdView *)view;
 ...
 */
- (void)didDismissModalViewForAd:(ACAdView *)view;
 ...
 * @param view The ad view sending the message.
 */
- (void)willLeaveApplicationFromAd:(ACAdView *)view;
